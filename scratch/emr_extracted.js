...
// === SCRIPT BLOCK 1 ===
    ...
    document.addEventListener("DOMContentLoaded", () => {
      // Auto-provision a mock token for prototype testing if none exists
      let token = sessionStorage.getItem("ehr_creds");
      if (!token) {
        token = "mock-token-123";
        sessionStorage.setItem("ehr_creds", token);
      }
      // Check Context
    ...
    function switchEmrView(viewId, pushState = true) {
      ...
      if(target) {
        ...
        // Let CSS handle display mode automatically to prevent layout collapse
