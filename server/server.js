...
function _loadSeed() {
  ...
  return { staff:[], patients:[], encounters:[], admissions:[], billing:[], facilities:[],
           orders:[], appointments:[], labResults:[], beds:[], consultations:[], referrals:[] };
}
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/v2/patients") {
    ...
    const patient = { id: nextId("PT", data.patients), name: body.name || "Unnamed", sex: body.sex || "Unknown", age: Number(body.age || 0), religion: body.religion || "", nationality: body.nationality || "", tribe: body.tribe || "", dateOfBirth: body.dateOfBirth || "", bloodGroup: body.bloodGroup || "", phone: body.phone || "", address: body.address || "", occupation: body.occupation || "", lga: body.lga || "", community: body.community || "", facilityId: body.facilityId || "FAC-PLSH", insurance: body.insurance || "Private Pay", risk: body.risk || "Routine", allergies: String(body.allergies || "").split(",").map(s => s.trim()).filter(Boolean), nextOfKin: body.nextOfKin || "", nextOfKinPhone: body.nextOfKinPhone || "", lastVisit: new Date().toISOString().slice(0, 10) };
 ...
 { sendJson(res, 200, data.orders); return; }
  ...
  // ── Referrals (Inter-Module HIE)
  if (req.method === "GET" && url.pathname === "/api/v2/referrals") {
    sendJson(res, 200, data.referrals || []);
    return;
  }
  if (req.method === "POST" && url.pathname === "/api/v2/referrals") {
    const body = await collectBody(req);
    if (!data.referrals) data.referrals = [];
    const referral = {
      id: nextId("REF", data.referrals),
      patientId: body.patientId,
      sourceModule: body.sourceModule || "PHC",
      targetModule: body.targetModule || "EHR",
      reason: body.reason || "Specialist Evaluation",
  ...
  if (req.method === "POST" && url.pathname === "/api/v2/phc/sync") {
    const body = await collectBody(req);
    if (!data.phc_records) data.phc_records = [];
    if (body.records && Array.isArray(body.records)) {
      body.records.forEach(record => {
        data.phc_records.unshift({
          ...record,
    ...
    sendJson(res, 200, { success: true, count: body.records ? body.records.length : 0 });
    return;
  }
  // ── Clinical Document Summary (CDA)
  if (req.method === "GET" && url.pathname === "/api/v2/reports/summary") {
    ...
    const summary = {
      ...
      encounters: (data.encounters || []).filter(e => e.patientId === patientId),
      labResults: (data.labResults || []).filter(l => l.patientId === patientId),
      prescriptions: (data.orders || []).filter(o => o.patientId === patientId && o.type === 'Pharmacy'),
      admissions: (data.admissions || []).filter(a => a.patientId === patientId)
    };
  ...
  // ── Orders / CPOE
  if (req.method === "GET" && url.pathname === "/api/v2/orders/all") {
  ...
  if (req.method === "POST" && url.pathname === "/api/v2/auth/register") {
    try {
      ...
      try {
        ...
        sendJson(res, 201, { token: "mock-jwt-token-12345", role: payload.role, name: newStaffName, userId: newStaffId, message: "Staff registered successfully" });
        return;
      } catch (dbErr) {
        const db = _readFile();
...
// ============================================================================
// Database Safety & Request Hardening
// ============================================================================
  ...
  try {
    ...
    if (req.method === "GET" && pathname === "/api/v2/legal/audit-matrix") {
      sendJson(res, 200, {
        ...
        jurisdiction: "State Ministry of Health & Federal Republic of Nigeria",
